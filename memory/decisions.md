...
## D13: Real Multi-User Accounts, Replacing the Shared Basic Auth Password

**Decision:** D11's single shared username/password is replaced with real per-user accounts (`/api/auth/signup`, still handled inside `middleware.js`, zero new Serverless Functions). Each account's credentials live in a **separate** private Vercel Blob store (`invoiser-accounts`, connected to the project as `ACCOUNTS_READ_WRITE_TOKEN`) from the app-data store (`invoiser-data` / `BLOB_READ_WRITE_TOKEN`) — a distinct security boundary, so a compromise of one token doesn't expose the other. The planned Google SSO ticket (#47) is dropped/closed in favor of this.

**Why a second Blob store, not a namespace in the existing one:** the user explicitly asked for account data to be isolated this way. Provisioned via `vercel storage create invoiser-accounts --type blob --access private` + `vercel storage connect invoiser-accounts --prefix ACCOUNTS --auth token --add-rw-token` (the CLI's storage commands, not the older `vercel blob create-store`, which can create a store but can't connect it under a custom env var name — it always tries `BLOB_READ_WRITE_TOKEN` and collides when one's already connected).

**The `@vercel/blob` SDK doesn't work in Edge Middleware (confirmed empirically, see D11's history):** so `api/_lib/blob-edge.js` reimplements just `get`/`put` as raw `fetch()` calls against Blob's actual HTTP contract, reverse-engineered from the SDK's own source (`get()` is a direct GET to the store's CDN host `https://<storeId>.private.blob.vercel-storage.com/<pathname>?cache=0`; `put()` is a PUT to Vercel's control-plane API at `https://vercel.com/api/blob/?pathname=...` with a handful of `x-vercel-blob-*`/`x-api-version` headers). Verified end-to-end against the real store (round-tripped a JSON write/read, confirmed a 404 path returns `null`) before wiring it into `middleware.js`.

**Per-user data isolation now runs through the session, not an env var:** `api/_lib/storage.js`'s `DEFAULT_USERNAME` fallback is gone — `readUserData`/`writeUserData` require an explicit username. Every `/api/*.js` function now independently derives it via `getSessionUsername(req)` (`api/_lib/session.js`, shared Web-Crypto HMAC/cookie code that runs unchanged in both the Edge middleware and the Node functions) and 401s if missing. This was chosen over having middleware forward the username as a request header, since that "rewrite-and-forward-a-header" trick is a Next.js-specific internal protocol (`x-middleware-request-*`) that a non-Next origin function doesn't parse — duplicating the cheap cookie-verify call in each function avoided depending on undocumented platform behavior for a security-relevant hand-off. `send-email.js`'s internal server-to-server fetch to `pdf.js` now explicitly forwards the `cookie` header, since that hop doesn't get it automatically.

**Passwords hashed with PBKDF2-SHA256 (Web Crypto `deriveBits`), not bcrypt/scrypt/argon2:** none of those are available in the Edge runtime, and login/signup need to run there (see D11's Serverless Function budget reasoning) — PBKDF2 via `crypto.subtle` is the strongest primitive available in both runtimes.

**The existing single user was migrated, not abandoned:** the "me" account (matching `users/me/...`, the pre-existing Blob data) was seeded into the new accounts store with its existing password, so the switchover didn't lock out the one real user mid-flight.

**Tradeoff accepted:** No password reset flow, no email verification, no rate limiting on login/signup attempts yet — usernames are trusted to self-select reasonably and abuse is a small-known-audience risk for now, same posture as D11. `middleware.js` logged a deprecation warning during this work ("edge" runtime deprecated in favor of Node.js Middleware) — not addressed here; if Node.js Middleware turns out to (a) still sit outside the 12-Function cap and (b) support the `@vercel/blob` SDK directly, it would let `blob-edge.js`'s hand-rolled HTTP client be replaced with the real SDK, which would be worth revisiting later but wasn't worth the risk of switching runtimes in the same change as shipping the auth rework.
